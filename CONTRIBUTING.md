...
3. **医疗领域 Agent 内容须附来源**：新增或修改 `agents/` 下医疗类 Agent 配置（专科知识、诊疗流程等表述）时，请在 PR 描述中注明权威依据（指南、教科书、共识文件等）。本项目对 AI 医疗输出的定位见 [DISCLAIMER.md](./DISCLAIMER.md)。
...
本项目采用 AGPL/商业双许可模式：为维持商业许可的有效性，首次提交实质性贡献（代码、文档、Agent 配置）时，需要在 PR 中确认同意 [CLA](#许可证)——即授予维护者将你的贡献包含在商业许可版本中再分发的权利。贡献本身始终以 AGPL-3.0 开源。
