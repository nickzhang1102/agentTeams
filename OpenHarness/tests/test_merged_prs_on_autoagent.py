...
Every test runs on ~/AutoAgent (override via OPENHARNESS_REAL_WORKSPACE) — a
17K LOC unfamiliar Python project.
...
MODEL = "kimi-k2.5"
WORKSPACE = Path(os.environ.get("OPENHARNESS_REAL_WORKSPACE", str(Path.home() / "AutoAgent")))
        ...
        try:
            ...
            jobs_data = [
                {"name": "lint-autoagent", "schedule": "0 */6 * * *",
                 "command": "cd /home/demo/AutoAgent && ruff check autoagent/"},
                {"name": "test-autoagent", "schedule": "0 2 * * *",
                 "command": "cd /home/demo/AutoAgent && python -m pytest tests/ -q"},
                {"name": "count-todos", "schedule": "0 9 * * 1",
                 "command": "cd /home/demo/AutoAgent && grep -r TODO autoagent/ | wc -l"},
